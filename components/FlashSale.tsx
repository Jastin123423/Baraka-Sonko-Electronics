...
const FlashSale: React.FC<FlashSaleProps> = ({ products, onProductClick, onSeeAllClick }) => {
  ...
  return (
    <div className="bg-white mb-2 py-3 px-3">
      ...
      <div className="flex overflow-x-auto no-scrollbar space-x-3">
        {products.map((p) => (
          <div key={p.id} className="flex-shrink-0 w-24 active:opacity-70 transition-opacity cursor-pointer" onClick={() => onProductClick(p)}>
